    ...
    if abs(fapmin - threshold) < accuracy and fapmin > threshold - accuracy:
        ...
        if abs(threshold - fapmax) < accuracy and fapmax < threshold + accuracy:
            ...
            while (
                ...
                print(i)
                SaveResults(
                    CW, rascension, declination, fpmid, fapmid, len(primpuls), save
                )
                print(f"Count: {count}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-n",
        "--len(primpuls)ber",
        help="How many pulsars to simulate? Will select for longest observation period from data.",
        type=int,
        default=14,
    )
    parser.add_argument(
        "-s",
        "--save",
        help="Where do you want the data? Defaults to CWD/Data/Sims",
    ...
    parser.add_argument(
        "--psi", help="SOurce Orientation variable", type=float, default=0
    )
    parser.add_argument(
        "--hmin",
    ...
    parser.add_argument(
        "-rpt", "--repeat", help="How many repetitions to run.", type=int, default=10
    )
    parser.add_argument(
        "--ptafile",
        help="If undefined, will create a pta using par and tim files. Can be defined to use an existing pickle file pta.",
        default=None,
    )
    parser.add_argument(
        "--timeline",
